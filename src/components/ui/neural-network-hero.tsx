...
function ShaderPlane() {
  ...
  return React.createElement('mesh', { ref: meshRef, position: [0, -0.75, -0.5] },
...
function ShaderBackground() {
  ...
  useGSAP(() => {
    ...
    gsap.to(canvasRef.current, { filter: 'blur(0px)', scale: 1, autoAlpha: 1, duration: 1.5, ease: 'power3.out', delay: 0.3 });
  }, { scope: canvasRef });
...
export default function Hero({
  ...
  const microItemsRefs = useRef<(HTMLLIElement | null)[]>([]);
  ...
  useGSAP(() => {
    ...
    gsap.set(headerRef.current, { filter: 'blur(16px)', y: 30, autoAlpha: 0, scale: 1.06 });
    ...
    tl.to(headerRef.current, { filter: 'blur(0px)', y: 0, autoAlpha: 1, scale: 1, duration: 0.9 }, 0.1);
    ...
    const microItems = microItemsRefs.current.filter(Boolean);
  ...
  return (
    <section ref={sectionRef} className={`relative min-h-screen w-full overflow-hidden flex flex-col ${className}`}>
      <ShaderBackground />
      <div className={`relative mx-auto flex max-w-7xl flex-col ${center ? 'items-center text-center' : 'items-start text-left'} gap-6 px-6 pb-24 pt-36 sm:gap-8 sm:pt-44 md:px-10 lg:px-16`}>
        {badgeText && (
        ...
        <h1 ref={headerRef} className={`max-w-4xl ${center ? 'text-center' : 'text-left'} text-5xl font-extralight leading-[1.05] tracking-tight text-white sm:text-6xl md:text-8xl ${titleClassName}`}>
          {title}
        </h1>
        {description && (
          <div ref={paraRef} className={`max-w-xl ${center ? 'text-center' : 'text-left'} text-base font-light leading-relaxed tracking-tight text-white/75 sm:text-lg`}>
            {description}
          </div>
        )}
        {ctaButtons.length > 0 && (
        ...
        {microDetails.length > 0 && (
          <ul className="mt-8 flex flex-wrap gap-6 text-xs font-extralight tracking-tight text-white/60">
            {microDetails.map((detail, index) => (
              <li key={index} ref={(el) => { microItemsRefs.current[index] = el; }} className="flex items-center gap-2">
